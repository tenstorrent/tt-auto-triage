...
#
# Handles two paths:
#   1. Cancellation: when .regression_handling/cancel.json exists and should_cancel=true
...
#
# Writes payload to .regression_handling/slack_payload.json and appends to GITHUB_OUTPUT:
#   payload_file=...
...
# Optional env:
#   MESSAGE_PATH (default: .regression_handling/output/slack_message.json)
...
MESSAGE_PATH="${MESSAGE_PATH-.regression_handling/output/slack_message.json}"
PAYLOAD_FILE=".regression_handling/slack_payload.json"
CANCEL_FILE=".regression_handling/cancel.json"
ERROR_MSG_FILE=".regression_handling/data/error_message.txt"
SUBJOB_RUNS_FILE=".regression_handling/data/subjob_runs.json"
JOB_OWNER_FILE=".regression_handling/data/job_owner.json"
...
if [ -f "$CANCEL_FILE" ]; then
  ...
  if [ "$SHOULD_CANCEL" = "true" ]; then
    RAW_MESSAGE=$(jq -r '.message // "Regression-handling cancelled"' "$CANCEL_FILE")
...
SLACK_DATA_DIR=".regression_handling/regression_handling/data"
