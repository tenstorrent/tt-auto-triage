...
#
# Run: cd .github/actions/slack-report-regression-analysis && bash tests/slack_report/payload_builder_test.sh
#

set -euo pipefail

REPO_ROOT="$(git rev-parse --show-toplevel)"
AT_ROOT="$REPO_ROOT/.github/actions/slack-report-regression-analysis"
...
mkdir -p "$tmpdir/.regression_analysis/data"
echo '{"should_cancel": true, "message": "Hardware check failed"}' > "$tmpdir/.regression_analysis/cancel.json"
echo "Error details here" > "$tmpdir/.regression_analysis/data/error_message.txt"
echo '[{"status":"failure","run_number":5,"job_url":"https://run.url","run_id":"555"}]' > "$tmpdir/.regression_analysis/data/subjob_runs.json"

cd "$tmpdir"
bash "$BUILD_SCRIPT"
cd - >/dev/null

payload=$(cat "$tmpdir/.regression_analysis/slack_payload.json")
...
rm -f "$tmpdir/github_output" "$tmpdir/.regression_analysis/slack_payload.json"
rm -f "$tmpdir/.regression_analysis/data/error_message.txt" "$tmpdir/.regression_analysis/data/subjob_runs.json"
echo '{"should_cancel": true, "message": "Cancelled"}' > "$tmpdir/.regression_analysis/cancel.json"
unset SLACK_TS
...
payload2=$(cat "$tmpdir/.regression_analysis/slack_payload.json")
...
cp "$SAMPLE_MSG" "$tmpdir/.regression_analysis/output/slack_message.json"
export MESSAGE_PATH="$tmpdir/.regression_analysis/output/slack_message.json"
export SLACK_TS=""
...
payload3=$(cat "$tmpdir/.regression_analysis/slack_payload.json")
...
payload4=$(cat "$tmpdir/.regression_analysis/slack_payload.json")
...
assert "Case 4 skips group resolution: no slack_groups.json fixture" [ ! -f "$tmpdir/.regression_analysis/data/slack_groups.json" ]
assert "Case 4 skips group resolution: no slack_directory.json fixture" [ ! -f "$tmpdir/.regression_analysis/data/slack_directory.json" ]
...
rm -rf "$tmpdir/.regression_analysis"
mkdir -p "$tmpdir/.regression_analysis/output" "$tmpdir/.regression_analysis/data"
cp "$SAMPLE_CASE5_MSG" "$tmpdir/.regression_analysis/output/slack_message.json"
export MESSAGE_PATH="$tmpdir/.regression_analysis/output/slack_message.json"
export SLACK_TS=""
export ALLOW_PINGS="true"
rm -f "$tmpdir/github_output"
...
payload5=$(cat "$tmpdir/.regression_analysis/slack_payload.json")
...
cat > "$tmpdir/.regression_analysis/data/job_owner.json" <<'EOF'
[
