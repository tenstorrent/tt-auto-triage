...
```bash
cd .github/actions/regression-analysis/regression_analysis
shellcheck regression_analysis.sh filter_triage.sh lib/hang_detect.sh lib/instructions_pipeline.sh get_triage_artifacts.sh
```
...
Also run the full suite via `.github/workflows/test-regression-analysis-lib.yml` (or `for t in tests/lib/*_test.sh; do bash "$t"; done`).
...
- **Workflow:** Dispatch or run the composite action on a branch with a hang-like failure (or synthetic `error_message.txt` + copied triage files in the triage workspace). Confirm:
  ...
  - Artifacts still include `regression-analysis-output` / `regression-analysis-data` as before.
