#!/bin/bash
#
# common.sh - Shared utilities for regression-analysis scripts
#
# Provides:
...
#
# Usage (from any script under regression_analysis/):
#   source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/common.sh"
#

# Guard against double-sourcing
if [ -n "${_REGRESSION_ANALYSIS_COMMON_LOADED:-}" ]; then
    return 0
fi
_REGRESSION_ANALYSIS_COMMON_LOADED=1

# ==============================================================================
# Root path resolution
# ==============================================================================
# regression_analysis/lib/common.sh  ->  lib dir is dirname, root is one level up.
_REGRESSION_ANALYSIS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REGRESSION_ANALYSIS_ROOT="${REGRESSION_ANALYSIS_ROOT:-$(cd "$_REGRESSION_ANALYSIS_LIB_DIR/.." && pwd)}"
...
# ==============================================================================
# Path helpers  (canonical paths relative to REGRESSION_ANALYSIS_ROOT)
# ==============================================================================

# get_data_dir([root]) -> path
# Returns path to regression_analysis/data directory.
get_data_dir()   { echo "${1:-$REGRESSION_ANALYSIS_ROOT}/regression_analysis/data"; }
# get_output_dir([root]) -> path
# Returns path to regression_analysis/output directory.
get_output_dir() { echo "${1:-$REGRESSION_ANALYSIS_ROOT}/regression_analysis/output"; }
# get_logs_dir([root]) -> path
# Returns path to regression_analysis/logs directory.
get_logs_dir()   { echo "${1:-$REGRESSION_ANALYSIS_ROOT}/regression_analysis/logs"; }
