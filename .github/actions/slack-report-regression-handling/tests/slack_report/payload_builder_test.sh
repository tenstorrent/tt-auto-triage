...
#
# Run: cd .github/actions/slack-report-regression-handling && bash tests/slack_report/payload_builder_test.sh
#
...
mkdir -p "$tmpdir/.regression_handling/data"
echo '{"should_cancel": true, "message": "Hardware check failed"}' > "$tmpdir/.regression_handling/cancel.json"
echo "Error details here" > "$tmpdir/.regression_handling/data/error_message.txt"
echo '[{"status":"failure","run_number":5,"job_url":"https://run.url","run_id":"555"}]' > "$tmpdir/.regression_handling/data/subjob_runs.json"

cd "$tmpdir"
bash "$BUILD_SCRIPT"
cd - >/dev/null

payload=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
rm -f "$tmpdir/github_output" "$tmpdir/.regression_handling/slack_payload.json"
rm -f "$tmpdir/.regression_handling/data/error_message.txt" "$tmpdir/.regression_handling/data/subjob_runs.json"
echo '{"should_cancel": true, "message": "Cancelled"}' > "$tmpdir/.regression_handling/cancel.json"
unset SLACK_TS
...
payload2=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
rm -rf "$tmpdir/.regression_handling"
mkdir -p "$tmpdir/.regression_handling/output" "$tmpdir/.regression_handling/data"
cp "$SAMPLE_MSG" "$tmpdir/.regression_handling/output/slack_message.json"
export MESSAGE_PATH="$tmpdir/.regression_handling/output/slack_message.json"
export SLACK_TS=""
...
payload3=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
rm -rf "$tmpdir/.regression_handling"
mkdir -p "$tmpdir/.regression_handling/output" "$tmpdir/.regression_handling/data"
cp "$SAMPLE_CASE4_MSG" "$tmpdir/.regression_handling/output/slack_message.json"
export MESSAGE_PATH="$tmpdir/.regression_handling/output/slack_message.json"
export SLACK_TS=""
...
payload4=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
assert "Case 4 skips group resolution: no slack_groups.json fixture" [ ! -f "$tmpdir/.regression_handling/data/slack_groups.json" ]
assert "Case 4 skips group resolution: no slack_directory.json fixture" [ ! -f "$tmpdir/.regression_handling/data/slack_directory.json" ]
...
rm -rf "$tmpdir/.regression_handling"
mkdir -p "$tmpdir/.regression_handling/output" "$tmpdir/.regression_handling/data"
cp "$SAMPLE_CASE5_MSG" "$tmpdir/.regression_handling/output/slack_message.json"
export MESSAGE_PATH="$tmpdir/.regression_handling/output/slack_message.json"
export SLACK_TS=""
export ALLOW_PINGS="true"
rm -f "$tmpdir/github_output"
...
payload5=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
cat > "$tmpdir/.regression_handling/data/job_owner.json" <<'EOF'
[
...
payload_owner=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
payload_owner_rep=$(cat "$tmpdir/.regression_handling/slack_payload.json")
...
text_id_only=$(jq -r '.text // empty' "$tmpdir/.regression_handling/slack_payload.json")
...
text_default_owner=$(jq -r '.text // empty' "$tmpdir/.regression_handling/slack_payload.json")
...
text_explicit_owner=$(jq -r '.text // empty' "$tmpdir/.regression_handling/slack_payload.json")
