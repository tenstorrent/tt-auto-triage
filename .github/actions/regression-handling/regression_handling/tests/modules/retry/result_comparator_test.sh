...
mkdir -p "$TMP_DIR/root/regression_handling/data"
echo "original error" > "$TMP_DIR/root/regression_handling/data/original_error.txt"
echo "retry error" > "$TMP_DIR/root/regression_handling/data/retry_error.txt"
...
run_copilot_error_comparison "$TMP_DIR/root" "$TMP_DIR/root/regression_handling/data" || true
comparison_file="$TMP_DIR/root/regression_handling/data/error_comparison.json"
assert_eq "run_copilot_error_comparison: produces error_comparison.json" "$([ -f "$comparison_file" ] && echo "exists" || echo "missing")" "exists"
same=$(get_same_failure_from_comparison "$TMP_DIR/root/regression_handling/data/error_comparison.json")
assert_eq "run_copilot_error_comparison: mock writes same_failure=true" "$same" "true"
extracted=$(get_retry_error_extracted "$TMP_DIR/root/regression_handling/data/error_comparison.json")
