#!/bin/bash
#
# config.sh - Centralised configuration for regression-handling scripts
#
...
if [ -n "${_REGRESSION_HANDLING_CONFIG_LOADED:-}" ]; then
    return 0
fi
_REGRESSION_HANDLING_CONFIG_LOADED=1

# config.sh depends on common.sh (REGRESSION_HANDLING_ROOT, get_data_dir, etc.)
_CONFIG_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=common.sh
source "$_CONFIG_LIB_DIR/common.sh"

# ==============================================================================
# Repository  (the repo being triaged, NOT the regression-handling repo itself)
# ==============================================================================
...
# Call once during script initialisation (filter_triage.sh, regression_handling.sh, etc.)
#
#   setup_triage_dirs [root]
#
# Sets globals: CANON_DATA_DIR, CANON_LOGS_DIR
setup_triage_dirs() {
    local root="${1:-$REGRESSION_HANDLING_ROOT}"
    ...
    # Convenience symlinks (./data -> regression_handling/data, etc.)
    ln -sfn regression_handling/data   "${root}/data"
    ln -sfn regression_handling/logs   "${root}/logs"
    ln -sfn regression_handling/output "${root}/output"
}
