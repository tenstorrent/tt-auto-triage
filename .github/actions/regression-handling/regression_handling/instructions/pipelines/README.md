...
One path per line, **relative to `regression_handling/`** (the tree with `instructions/` and `lib/`). Blank lines and `#` comments are ignored.

| File | Used by |
|------|---------|
| `filter.fragments` | `filter_triage.sh` |
| `main.fragments` | `regression_handling.sh` |
...
- **Trigger** — bash function `trigger_name "$data_dir"`; exit **0** to run the follow-up. Define in `lib/*.sh` and **source from `regression_handling.sh`** (see `hang_detect.sh`).
- **Path** — relative to `regression_handling/`; file must exist when the trigger fires.
