...
#
# Source from regression_analysis.sh. To add another follow-up type: define should_run_* here (or
# another sourced lib), add a line to instructions/pipelines/followups.manifest, and source
# that lib from regression_analysis.sh if needed.
#

if [ -n "${_REGRESSION_ANALYSIS_HANG_DETECT_LOADED:-}" ]; then
    return 0
fi
_REGRESSION_ANALYSIS_HANG_DETECT_LOADED=1
