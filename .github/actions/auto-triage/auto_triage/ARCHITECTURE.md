...
Auto-triage is a composite GitHub Action that automatically analyzes CI failures in `tt-metal`, identifies likely root causes, retries deterministic failures on supported hardware, and reports results to Slack.
...
│   ├── hang_detect.sh      # Detect card hang for conditional hang LLM instructions
...
│   └── modules/
│
├── auto_triage.sh          # Main triage entry point
...
aggregate-workflow-data.yaml (caller)
   ...
   action.yml (composite action)
        ...
        │
        ├─► auto_triage.sh ──► llm_runner (Copilot analysis)
        ...
        │
        └─► slack-report-auto-triage (separate action)
...
Run all tests: see `.github/workflows/test-auto-triage-lib.yml`.
