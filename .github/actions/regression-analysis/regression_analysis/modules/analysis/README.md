...
- **`followups.manifest`** — `trigger_function` then whitespace then instruction path (rest of line); each matching trigger runs an extra `run_llm_analysis`. Define triggers in `lib/*.sh` and **source them from `regression_analysis.sh`** (see `hang_detect.sh`).
