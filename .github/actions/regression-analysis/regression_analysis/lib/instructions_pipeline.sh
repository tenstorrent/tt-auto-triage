...
if [ -n "${_REGRESSION_ANALYSIS_INSTRUCTIONS_PIPELINE_LOADED:-}" ]; then
    return 0
fi
_REGRESSION_ANALYSIS_INSTRUCTIONS_PIPELINE_LOADED=1

_IP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=common.sh
source "$_IP_DIR/common.sh"

# Paths are relative to the regression_analysis/ root (directory containing instructions/ and lib/).
...
run_instruction_followups() {
    ...
    while IFS= read -r raw || [[ -n "$raw" ]]; do
        ...
        if ! declare -F "$trigger" &>/dev/null; then
            log_warn "followups.manifest: unknown trigger '$trigger' (define it and source from regression_analysis.sh)"
            continue
        fi
