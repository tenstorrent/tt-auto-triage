#!/bin/bash
#
# Tests for regression_analysis.sh
#
# Run: cd .github/actions/regression-analysis/regression_analysis && ./tests/modules/analysis/regression_analysis_test.sh
#
...
REGRESSION_ANALYSIS="$AT_ROOT/regression_analysis.sh"

echo "=== regression_analysis.sh ==="

# -- missing args -------------------------------------------------------------
set +e
out=$(bash "$REGRESSION_ANALYSIS" 2>&1)
rc=$?
set -e
...
set +e
out=$(bash "$REGRESSION_ANALYSIS" "workflow" 2>&1)
rc=$?
set -e
...
# -- LLM delegation: ensure regression_analysis.sh invokes copilot ----------------------
# This is a unit-style test: it stubs a `copilot` executable in PATH, prepares
# minimal data/instructions, runs regression_analysis.sh, and asserts the stub ran.
...
# Prepare minimal subjob_runs.json. Use canonical path (regression_analysis/data) because
# setup_triage_dirs overwrites ./data with a symlink to regression_analysis/data.
DATA_DIR="$AT_ROOT/regression_analysis/data"
mkdir -p "$DATA_DIR"
...
# regression_analysis.sh requires instructions_for_llm.txt (not instructions.md). Back it up
...
echo "Test instructions for regression_analysis LLM analysis." > "$INSTRUCTIONS_FILE"

# Run regression_analysis.sh with two arguments so it takes the main path.
set +e
bash "$REGRESSION_ANALYSIS" "dummy-workflow" "test-subjob" >/dev/null 2>&1
...
assert "regression_analysis delegates to run_llm_analysis (copilot invoked)" [ -s "$COPILOT_STUB_LOG" ]
