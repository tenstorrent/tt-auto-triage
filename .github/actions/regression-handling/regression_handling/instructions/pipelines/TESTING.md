...
```bash
cd .github/actions/regression-handling/regression_handling
shellcheck regression_handling.sh filter_triage.sh lib/hang_detect.sh lib/instructions_pipeline.sh get_triage_artifacts.sh
```
...
Also run the full suite via `.github/workflows/test-regression-handling-lib.yml` (or `for t in tests/lib/*_test.sh; do bash "$t"; done`).
...
- **Workflow:** Dispatch or run the composite action on a branch with a hang-like failure (or synthetic `error_message.txt` + copied triage files in the triage workspace). Confirm:
  ...
  - Artifacts still include `regression-handling-output` / `regression-handling-data` as before.
