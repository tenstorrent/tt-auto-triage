...
Regression-handling is a composite GitHub Action that automatically analyzes CI failures in `tt-metal`, identifies likely root causes, retries deterministic failures on supported hardware, and reports results to Slack.
...
│   ├── hang_detect.sh      # Hang follow-up trigger (sourced from regression_handling.sh; see followups.manifest)
...
│   └── modules/
│
├── regression_handling.sh          # Main triage entry point
...
aggregate-workflow-data.yaml (caller)
   ...
   action.yml (composite action)
        ...
        │
        ├─► regression_handling.sh ──► llm_runner (Copilot analysis)
        ...
        │
        └─► slack-report-regression-handling (separate action)
...
Run all tests: see `.github/workflows/test-regression-handling-lib.yml`.
