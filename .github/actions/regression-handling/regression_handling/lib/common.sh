#!/bin/bash
#
# common.sh - Shared utilities for regression-handling scripts
#
# Provides:
...
#
# Usage (from any script under regression_handling/):
#   source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/common.sh"
#

# Guard against double-sourcing
if [ -n "${_REGRESSION_HANDLING_COMMON_LOADED:-}" ]; then
    return 0
fi
_REGRESSION_HANDLING_COMMON_LOADED=1

# ==============================================================================
# Root path resolution
# ==============================================================================
# regression_handling/lib/common.sh  ->  lib dir is dirname, root is one level up.
_REGRESSION_HANDLING_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REGRESSION_HANDLING_ROOT="${REGRESSION_HANDLING_ROOT:-$(cd "$_REGRESSION_HANDLING_LIB_DIR/.." && pwd)}"
...
# ==============================================================================
# Path helpers  (canonical paths relative to REGRESSION_HANDLING_ROOT)
# ==============================================================================

# get_data_dir([root]) -> path
# Returns path to regression_handling/data directory.
get_data_dir()   { echo "${1:-$REGRESSION_HANDLING_ROOT}/regression_handling/data"; }
# get_output_dir([root]) -> path
# Returns path to regression_handling/output directory.
get_output_dir() { echo "${1:-$REGRESSION_HANDLING_ROOT}/regression_handling/output"; }
# get_logs_dir([root]) -> path
# Returns path to regression_handling/logs directory.
get_logs_dir()   { echo "${1:-$REGRESSION_HANDLING_ROOT}/regression_handling/logs"; }
